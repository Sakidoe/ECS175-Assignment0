...
class Shape
{
    ...
    createVAO( gl, shader )
    {
        ...
        //bind the VAO
        gl.bindVertexArray(this.vertex_array_object); // bind the VAO
        //bind VBO
        gl.bindBuffer(gl.ARRAY_BUFFER, this.vertices_buffer); //given in constructor for gl
        //link it to shader attribute 'a_position'
        const positionAttributeLocation = shader.getAttributeLocation("a_position"); // shader.js maybe
        //setting correct vertex attrib pointer settings for VBO
        gl.enableVertexAttribArray(positionAttributeLocation);
        //maps correctly to a_pos
        gl.vertexAttribPointer(positionAttributeLocation, this.num_components, gl.FLOAT, false, 0, 0); // also in textbook
        //clean
    ...
    createVBO( gl )
    {
        ...
        //fill with this.vertices and then binds  -bufferData(target, srcData, usage);
    ...
    render( gl )
    {
        ...
        //basically draw in the examples?
        //clears the sceen
...
class Triangle extends Shape
{
    ...
    constructor( gl, shader, position, color, sideLength) 
    {
        ...
        let vertices = [
            // bottom
            x, y - halfSideLength * sinangle * 2,
            // top right
            x + halfSideLength, y + halfSideLength * sinangle,
        ...
        super( gl, shader, vertices, indices, color, gl.TRIANGLES, indices.length, {a_position: 0} );
...
class WebGlApp 
{
    ...
    constructor()
    {
        ...
        //adding in key listeners, I'm pretty sure theres an input.js but confused fr
        //if x, clear shapes
        //render again
        ...
        // if mouseclick, add triangle
        window.addEventListener('click', (e) => {
            const x = e.clientX;
            const y = e.clientY;
            this.addTriangle(this.gl, this.shader, [x, y], 50); // Change the '50' to the desired side length
            this.render();
        });
    ...
    clearCanvas( gl )
    {
        ...
        const [r,g,b] = hex2rgb("#022851"); // returns an array for RGB
        gl.clearColor(r,g,b, 1.0);  //clearColor(red, green, blue, alpha(?))
    ...
    {
        const color = [1.0, 0.729, 0.0]; //idk if hex2rgb can still work here
